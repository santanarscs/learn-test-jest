...
describe('Tools reducer', () => {
	...
	it('@tools/GET_TOOLS_REQUEST', () => {
		...
		expect(state).toStrictEqual({ data: [], loading: true });
	});
	it('@tools/GET_TOOLS_SUCCESS', () => {
		const state = reducer(INITIAL_STATE, Tools.getToolsSuccess(tools));
		expect(state).toStrictEqual({ data: tools, loading: false });
	});
	...
	it('@tools/ADD_TOOL_SUCCESS', () => {
		...
		expect(state).toStrictEqual({ data: [tools[0]], loading: false });
	});
	it('@tools/REMOVE_TOOL_REQUEST', () => {
		const state = reducer({ data: tools, loading: false }, Tools.removeToolRequest(1));
		expect(state).toStrictEqual({ data: tools.filter(tool => tool.id !== 1), loading: true });
	});
	it('@tools/REMOVE_TOOL_SUCCESS', () => {
		const state = reducer(INITIAL_STATE, Tools.removeToolSuccess());
		expect(state).toStrictEqual({ data: [], loading: false });
	});
});
